...
16. 스플래시에서 첫 화면으로 넘어갈 때의 색. 다크에서 창 배경(AppCompat `#303030`)과 canvas(`#030712`)가 다르다(8.3의 010)
17. 천 단위 쉼표를 넣는 금액 칸의 커서. 쉼표 바로 뒤에서 지울 때와 가운데 숫자를 고칠 때
18. 칩과 입력 칸 글자의 세로 가운데(Android `includeFontPadding`)
19. 히어로 안 간격이 모두 12라 라벨·금액, 게이지·캡션 묶음이 읽히는지
...
1. **남은 예산**: "9월 남은 예산"(sm muted) → 금액(2xl) → 소비 속도 한 줄(md, 할 수 있는 일이라 금액 바로 밑) → 예산 게이지(오늘 세로선) → "788,400원 사용 · 66% | 예산 1,200,000원"(sm muted, 좁으면 두 줄) → "세로선은 오늘이에요 · 이번 달 80% 지남"(sm muted, 세로선의 뜻). 달은 헤더("9월")가 맡으므로 12장 6번에서 헤더를 붙이면 제목을 "남은 예산"(초과면 "예산")으로 줄인다. 시안은 헤더가 없어 달을 제목에 둔다. 총예산을 넘으면 제목이 "9월 예산", 금액이 "18,000원 초과"(danger), 소비 속도가 "남은 6일 · 지금부터 쓰는 만큼 초과가 늘어요", 게이지가 꽉 찬 danger다("홈 초과" 시안). 예산이 없으면 "아직 예산이 없어요" + "예산 정하기"
