...
- 새로 쓴 `className`이 화면에 안 먹으면 Metro를 다시 켭니다(`pnpm start`). Tailwind 결과는 다른 파일의 클래스에 달려 있는데 Metro는 켜져 있는 동안 global.css 내용이 바뀔 때만 다시 변환합니다. CSS 결과는 디스크 캐시에 남지 않아 `--reset-cache`는 필요 없습니다([docs/DESIGN.md](docs/DESIGN.md) 1.4).
