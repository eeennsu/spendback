...
2026-09-24 기획 인터뷰 세션에서 작성하고, 같은 날 1단계 검증 세션에서 갱신했다. 이 문서는 다음 세션이 처음 읽는 문서다.
...
## 1단계: 검증 (2026-09-24 클라우드 세션, 대부분 완료)
...
1. **기기 확인**: 클라우드 세션이라 하지 못했다. 폰을 USB로 연결하고 `adb shell getprop ro.soc.model`, `adb shell getprop ro.product.model`로 칩을 확인해 PRD 6장·10장·11장의 Exynos 가정을 확정한다.
2. **모델 파일 확인**: 클라우드 세션에서는 Hugging Face가 막혀 있었다. PRD 6장 레지스트리의 세 저장소에서 Q4_K_M 파일명, 정확한 크기, SHA-256을 확인해 PRD 6장에 적는다.
3. **보류된 PRD 수정안**(사용자가 아직 승인하지 않았다. 필요해지는 시점에 제안한다)
   ...
   - 7장에 모델 레지스트리 엔티티(id, 저장소, 파일명, 크기, sha256, 라이선스) 추가
   - GBNF에서 전각 숫자와 JSON `\u` 이스케이프 차단
   - 4.1 "결제 취소 시 삭제" 규칙을 2단계 카드 알림으로 이동
4. 위 1·2를 PRD에 반영하면 2단계로 넘어간다.
